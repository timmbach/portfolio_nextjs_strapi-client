...
function ContactMe({ contacts }: IPropTypes) {
  ...
  return (
    <div className="h-screen flex flex-col relative text-center md:text-left xl:flex-row max-w-[2000px] xl:px-10 justify-center mx-auto items-center z-0">
      ...
      <div className="flex flex-col space-y-7 items-center pt-16">
        <h4 className="text-2xl md:text-3xl px-6 md:px-0 font-semibold text-center">
        ...
        <div className="space-y-3">
          <a href={`tel:${+contacts.items[0].attributes.phone}`}>
            <div className="flex items-center space-x-5 justify-center w-max mx-auto">
              <PhoneIcon className="text-[#f7ab0a] h-7 w-7 animate-pulse" />
              <p className="text-2xl">{contacts.items[0].attributes.phone}</p>
            </div>
          </a>
          <a href={`mailto:${contacts.items[0].attributes.email}`}>
            <div className="flex items-center space-x-5 mt-2 justify-center w-max mx-auto">
              <EnvelopeIcon className="text-[#f7ab0a] h-7 w-7 animate-pulse" />
              <p className="text-2xl">{contacts.items[0].attributes.email}</p>
            </div>
          </a>
          <div className="flex items-center space-x-5 justify-center w-max mx-auto">
            <MapPinIcon className="text-[#f7ab0a] h-7 w-7 animate-pulse" />
            <p className="text-2xl">{contacts.items[0].attributes.address}</p>
          </div>
        </div>
